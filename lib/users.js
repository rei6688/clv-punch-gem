...
async function register(username, password) {
  ...
  if (await isDbAvailable()) {
    try {
      ...
      // Create session in the same invocation so we don't need a separate login call
      const row = await dbQueryOne(`SELECT id FROM users WHERE username = ?`, [username]);
      if (row) {
        await dbExec(
  ...
  const id = _nextId++;
  _users.set(username, { id, username, password_hash: hash });
  _sessions.set(token, { user_id: id, expires_at: expires });
  return { ok: true, token };
}
